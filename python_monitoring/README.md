...
`dashboard.py` 상단의 다음 값이 고정 분석 대상입니다.

```python
TARGET_VIDEO_URL = "https://www.youtube.com/watch?v=fNHLffyXnQM"
```

다른 영상을 분석하려면 이 한 줄만 변경해 GitHub에 커밋하세요. Streamlit 화면에서는 주소 입력 없이 사이드바의 `데이터 수집·분석` 버튼을 누릅니다.

Gemini 모델은 Streamlit Secrets의 다음 값으로 변경할 수 있습니다.

```toml
GEMINI_MODEL = "gemini-2.5-flash"
```

`GEMINI_API_KEY`는 YouTube용 Google Cloud 키가 아니라 Google AI Studio에서 생성한 Gemini Auth key여야 합니다.
