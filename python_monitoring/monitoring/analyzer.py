...
class GeminiResponseError(ValueError):
    """Raised when Gemini returned content that cannot be stored safely."""


_CATEGORY_ALIASES = {
    "sentiment": {
        "positive": "긍정", "neutral": "중립", "negative": "부정",
        "긍정적": "긍정", "중립적": "중립", "부정적": "부정",
    },
    "risk": {
        "observe": "관찰", "observation": "관찰", "caution": "주의",
        "warning": "주의", "urgent": "긴급", "emergency": "긴급",
    },
}


def _clean_json_text(text: str) -> object:
    value = text.strip()
    if value.startswith("```"):
...
def _normalize_item(value: object) -> AnalysisItem:
    if not isinstance(value, dict):
        raise GeminiResponseError("Gemini 분석 항목이 객체 형식이 아닙니다.")
    item = dict(value)

    for field, aliases in _CATEGORY_ALIASES.items():
        raw = item.get(field)
        if isinstance(raw, str):
            item[field] = aliases.get(raw.strip().lower(), raw.strip())

    for field, maximum in (("keyword", 30), ("rationale", 160)):
        raw = item.get(field)
        if isinstance(raw, str):
            item[field] = raw.strip()[:maximum]

    confidence = item.get("confidence")
    if isinstance(confidence, str):
        try:
            confidence = float(confidence.rstrip("%"))
            if "%" in item["confidence"]:
                confidence /= 100
    ...
    spam = item.get("spam")
    if isinstance(spam, str) and spam.strip().lower() in {"true", "false"}:
        item["spam"] = spam.strip().lower() == "true"

    try:
        return AnalysisItem.model_validate(item)
    except ValidationError as exc:
        raise GeminiResponseError("Gemini 분석 항목의 필드값이 허용 범위와 일치하지 않습니다.") from exc


def _parse_batch(response: object, expected: set[str]) -> AnalysisBatch:
    parsed = getattr(response, "parsed", None)
    try:
        if isinstance(parsed, AnalysisBatch):
            batch = parsed
        else:
            payload = parsed
            if not isinstance(payload, (dict, list)):
                ...
                if not text:
                    raise GeminiResponseError("Gemini가 분석 JSON을 반환하지 않았습니다.")
                payload = _clean_json_text(text)
            values = payload.get("items") if isinstance(payload, dict) else payload
            if not isinstance(values, list):
                raise GeminiResponseError("Gemini 분석 JSON에 items 배열이 없습니다.")
            batch = AnalysisBatch(items=[_normalize_item(value) for value in values])
    except (ValidationError, json.JSONDecodeError, TypeError) as exc:
        raise GeminiResponseError("Gemini 분석 JSON이 지정된 형식과 일치하지 않습니다.") from exc

    actual = [item.comment_id for item in batch.items]
    if len(actual) != len(expected) or set(actual) != expected:
        raise GeminiResponseError("Gemini 응답의 댓글 ID가 입력과 1:1로 일치하지 않습니다.")
    return batch
    ...
    def analyze(self, rows: list[dict]) -> list[AnalysisItem]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                batch = _parse_batch(response, expected)
                return batch.items
            except Exception as exc:
                last_error = exc
                message = str(exc).lower()
                code = getattr(exc, "code", None)
                response_error = isinstance(exc, GeminiResponseError)
                retryable = response_error or any(
                    token in message for token in ("429", "timeout", "tempor", "500", "502", "503", "504")
                )
                if not retryable or attempt + 1 >= self.max_retries:
                    if response_error:
                        if len(rows) > 1:
                            midpoint = len(rows) // 2
                            return self.analyze(rows[:midpoint]) + self.analyze(rows[midpoint:])
                        detail = (
                            "Gemini가 단일 댓글에도 유효한 분석 JSON을 반환하지 않았습니다. "
                            "잠시 후 다시 시도하거나 GEMINI_MODEL 설정을 확인하세요."
                        )
                    elif "api key not valid" in message or "api_key_invalid" in message:
