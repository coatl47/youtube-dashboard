...
def _parse_batch(response: object, expected: set[str]) -> AnalysisBatch:
    ...
    try:
        if isinstance(parsed, AnalysisBatch):
            ...
            batch = AnalysisBatch.model_validate(parsed)
        elif isinstance(parsed, list):
            batch = AnalysisBatch.model_validate({"items": parsed})
        else:
            text = getattr(response, "text", None)
            if not text:
                raise GeminiResponseError("Gemini가 분석 JSON을 반환하지 않았습니다.")
            batch = AnalysisBatch.model_validate_json(text)
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if not retryable or attempt + 1 >= self.max_retries:
                    if response_error:
                        detail = (
                            "Gemini 응답 형식이 분석 스키마와 일치하지 않습니다. "
                            "ANALYSIS_BATCH_SIZE를 5로 낮춘 뒤 다시 시도하세요."
                        )
