...
from monitoring.analyzer import GeminiResponseError, _parse_batch
...
class GeminiResponseTests(unittest.TestCase):
    def test_accepts_sdk_dict_response(self):
        response = SimpleNamespace(parsed={"items": [{
            ...
            "confidence": 0.91,
            "risk": "관찰",
            "rationale": "설명에 대한 긍정 반응",
        }]})
        batch = _parse_batch(response, {"comment-1"})
        self.assertEqual(batch.items[0].comment_id, "comment-1")

    def test_rejects_mismatched_comment_ids(self):
        response = SimpleNamespace(parsed={"items": []})
        with self.assertRaises(GeminiResponseError):
            _parse_batch(response, {"comment-1"})

    def test_normalizes_common_sdk_values_and_fenced_json(self):
        response = SimpleNamespace(parsed=None, text='''```json
        {"items": [{
          "comment_id": "comment-1", "sentiment": "positive",
          "target": "국민연금", "topic": "정보 공개",
          "keyword": "아주 긴 키워드입니다 아주 긴 키워드입니다 아주 긴 키워드입니다",
          "spam": "false", "confidence": "91%", "risk": "observe",
          "rationale": "설명에 대한 긍정 반응"
        }]}
        ```''')
        item = _parse_batch(response, {"comment-1"}).items[0]
        self.assertEqual(item.sentiment, "긍정")
        self.assertEqual(item.risk, "관찰")
        self.assertFalse(item.spam)
        self.assertEqual(item.confidence, 0.91)
        self.assertLessEqual(len(item.keyword), 30)
