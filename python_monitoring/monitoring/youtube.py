...
class YouTubeRequestError(RuntimeError):
    """Safe API error that never includes the credential-bearing request URL."""
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if status not in {429, 500, 502, 503, 504} or attempt + 1 >= self.max_retries:
                    ...
                    if "api key not valid" in message or "keyinvalid" in message:
                        detail = "YOUTUBE_API_KEY가 유효하지 않습니다. 새 키로 교체하세요."
                    elif "accessnotconfigured" in message or "has not been used" in message:
                        detail = "Google Cloud 프로젝트에서 YouTube Data API v3를 활성화하세요."
                    elif "quota" in message:
                        detail = "YouTube Data API 일일 할당량을 확인하세요."
                    elif "commentsdisabled" in message:
                        detail = "해당 영상은 댓글이 비활성화되어 있습니다."
                    else:
                        detail = f"YouTube API 요청이 거부되었습니다 (HTTP {status or 'unknown'})."
                    raise YouTubeRequestError(detail) from None
